...
export class AIPatternFunctions {
  ...
  public async fillGrid(): Promise<PatternFunctionResult> {
    ...
    for (let i = 0; i < newGrid.length && tileIndex < availableTiles.length; i++) {
      // Only fill empty cells - DO NOT touch cells that already have tiles
      if (!newGrid[i].tile) {
        newGrid[i] = {
          ...newGrid[i],
          tile: availableTiles[tileIndex],
          rotation: newGrid[i].rotation || 0 // Keep existing rotation or default to 0
        };
        tileIndex++;
        filledCount++;
      }
      // If cell already has a tile, skip it completely - don't modify it
    }
